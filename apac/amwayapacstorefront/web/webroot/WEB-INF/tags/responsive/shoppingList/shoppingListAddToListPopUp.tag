...
<%@ attribute name="entry" required="true" type="com.amway.facades.product.data.WishlistEntryData" %>
 ...
 <product:productPrimaryImage product="${entry.product}" format="quickView" cssClass="cart-popup__thumbnail"/>
    <div class="cart-popup__item-detail">
        ...
        <div class="cart-popup__quantity cart-popup__item-retailprice">
            <span class="cart-popup__qty no-right-border"><spring:theme code="pdp.add.to.cart.popup.label.quantity" arguments="${1}" /></span>
        </div>
    </div>
</div>