...
public class ResourceSolrDocumentSearchResultValuePopulator implements
		Populator<DocumentData<SearchQuery, Document>, SearchResultValueData>
{
	...
	{
		//		final String fieldName = "id".equals(property.getName()) || "displayImage".equals(property.getName()) ? property.getName()
		//				: translateFieldName(source.getSearchQuery(), property);
		final Object result = source.getDocument().getFieldValue(property.getName());
		return property.isMultiValue() && result != null ? ((ArrayList<String>) result).get(0) : result;
	}
