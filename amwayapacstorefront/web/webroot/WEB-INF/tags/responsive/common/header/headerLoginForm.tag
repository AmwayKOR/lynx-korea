...
<div class="popover fade bottom extraclass1 no-login collapse <c:if test="${loginError}">in</c:if>" role="tooltip" id="login-drop-content" style="margin-top: 31px;">
    ...
    <div class="popover-content">
        ...
        <form class="topbar__popover login-form authentication-js" action="${login_auth_url}" method="post" >
            <h1 class="login-form__title"><spring:theme code="text.header.loginform.title" /></h1>
            <p class="login-form__sub-text"><spring:theme code="text.header.loginform.sub-text" /></p>
            <c:if test="${loginError}"><p class="red"><spring:theme code="text.header.loginform.incorrectcredentials" /></p></c:if>
            ...
            <label for="remember" class="remember-me"><spring:theme code="text.header.loginform.remember" /></label>
            <button class="btn-primary button login-form__submit" type="submit" value="submit"><spring:theme code="text.header.loginform.submit" /></button>
            <a class="login-form__link" href="#"><spring:theme code="text.header.loginform.forgotpassword" /></a>
            <a class="login-form__link" href="#"><spring:theme code="text.header.loginform.firstlogging" /></a>
            <hr>
            <p class="login-form__owner"><spring:theme code="text.header.loginform.owner" /></p>
