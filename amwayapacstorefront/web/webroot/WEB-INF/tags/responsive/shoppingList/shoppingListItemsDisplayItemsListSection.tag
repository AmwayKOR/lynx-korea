...
<c:url var="productGridAddToCartUrl" value="/cart/addGrid" />
<ul class="shopping-cart-item-list js-shopping-cart-item-list">
	<li class="visible-md visible-lg">
		<div class="col-xs-12 list-header">
			...
			<div class="col-xs-2 list-item-quantity">
				<spring:theme code="shoppinglist.items.section.table.qty.label" />
			</div>
			<div class="col-xs-2 list-item-price">
				<spring:theme code="shoppinglist.items.section.table.totalprice.label" />
			</div>
			<div class="col-xs-2 list-item-total">
				<spring:theme code="shoppinglist.items.section.table.totalpvbv.label" />
			</div>
		</div>
	</li>
	<c:forEach items="${shoppingListData.entries}" var="shoppingListEntry" varStatus="loop">
		...
		<li class="product-list-item js-shopping-cart-list-item shopping-list-entry">
			<form:form action="${productGridAddToCartUrl}" commandName="addToCartOrderForm" cssClass="grid-add-to-cart-form">
				<form:hidden path="cartEntries[${loop.index}].productCode" value="${shoppingListEntry.product.code}" />
				<div class="col-xs-12 col-md-6 product-details print-col-6">
					...
					<div class="product-item-element list-item-image">
						...
						<div class="list-item-remove">
							<div class="remove-item-btn js-remove-entry-button" id="removeEntry_${loop.index}">
								<span class="remove-product-from-shopping-list" data-shopping-list-uid="${shoppingListData.uid}"
									data-product-code="${shoppingListEntry.product.code}" data-remove-url="${removeProductUrl}">
									<spring:theme code="shoppinglist.items.section.table.item.remove.label" />
								</span>
							</div>
						</div>
					</div>
					<div class="product-item-element list-item-info">
						<a href="${productUrl}">
							<span class="product-name">${shoppingListEntry.product.name}</span>
						</a>
						<div class="product-code">
							<spring:theme code="shoppinglist.items.section.table.item.itemnumber.label" />
							<span class="product-item-number">${shoppingListEntry.product.code}</span>
						</div>
						<c:if
							test="${(not empty shoppingListEntry.product.categories) and (fn:length(shoppingListEntry.product.categories) > 0)}">
				...
				<div class="col-xs-9 product-item-element list-item-ibo-price col-md-2 list-item-price print-col-6">
					<span class="price-label">
						<spring:theme code="shoppinglist.items.section.table.item.price.label" />
					</span>
					<span class="value-wrapper price-value-wrapper" data-price="${shoppingListEntry.product.price.value}"
				...
				<div class="col-xs-9 col-md-2 product-item-element list-item-total js-item-total print-col-6">
					...
					<span class="value-wrapper pv-bv-value-wrapper" data-pv="${shoppingListEntry.product.price.amwayValue.pointValue}"
