...
/**
 * Populator implementation for {@link AddressModel} as source and {@link CisAddress} as target type.
 */
public class AmwayDMSAddressPopulator implements Populator<AddressModel, CisAddress>
{
	@Override
	...
	protected String getRegionIsoCode(final AddressModel source)
	{
		if (source.getRegion() != null && StringUtils.isNotEmpty(source.getRegion().getIsocode()))
		{
			...
			//CIS needs only second part of region isocode
			if (regionIsocode.startsWith(source.getCountry().getIsocode()))
			{
