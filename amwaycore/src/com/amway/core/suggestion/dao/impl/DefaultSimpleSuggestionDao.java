 ...
 *  
 */
package com.amway.core.suggestion.dao.impl;
...
import com.amway.core.suggestion.dao.SimpleSuggestionDao;
...
public class DefaultSimpleSuggestionDao extends AbstractItemDao implements SimpleSuggestionDao
{
	...
	private static final String REF_QUERY_CATEGORY_START =
			"SELECT {p.PK}" + " FROM {Product AS p" + " LEFT JOIN ProductReference AS r ON {p.PK}={r.target}"
					+ " LEFT JOIN OrderEntry AS e ON {r.source}={e.product}" + " LEFT JOIN Order AS o ON {e.order}={o.PK}"
	...
	private static final String REF_QUERY_SUB =
			" AND NOT EXISTS ({{" + " SELECT 1 FROM {OrderEntry AS e2 LEFT JOIN Order AS o2 ON {e2.order}={o2.PK} } "
