...
<ycommerce:testId code="product_wholeProduct">
	<div class="productGridItem ${hasPromotion ? 'productGridItemPromotion' : ''}">
		<a href="${productUrl}" title="${product.name}" class="productMainLink">
  		...
  		<c:choose>
  			<c:when test="${product.serviceProductFlag}">
	...
	<div class="cart clearfix">
		...
		<c:url value="/cart/add" var="addToCartUrl"/>
		 <form:form id="addToCartForm${product.code}" action="${addToCartUrl}" method="post" class="add_to_cart_form clear_fix">
			...
			<c:if test="${not empty product.averageRating && not product.serviceProductFlag}">
				<product:productStars rating="${product.averageRating}" />
			</c:if>
			<ycommerce:testId code="product_addProduct_button">
				<button type="${buttonType}" class="addToCartButton <c:if test="${product.stock.stockLevelStatus.code eq 'outOfStock' }">out-of-stock</c:if>" <c:if test="${product.stock.stockLevelStatus.code eq 'outOfStock' || product.serviceProductFlag }"> disabled="disabled" aria-disabled="true"</c:if>>${addToCartText}</button>
			</ycommerce:testId>
		</form:form>
