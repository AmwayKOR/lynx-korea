 ...
 */
public class AmwayDMSReverseAddressPopulator implements Populator<CisAddress, AddressModel>
{
	private static final Logger LOG = Logger.getLogger(AmwayDMSReverseAddressPopulator.class);
	...
	public void populate(final CisAddress source, final AddressModel target) throws ConversionException
	{
		if (source != null)
		{
			...
			if (source.getState() != null && source.getState().length() == REGION_ISOCODE_LENGTH)
			{
				try
				{
					final RegionModel regionModel = getI18nService()
							.getRegion(countryModel, source.getCountry() + "-" + source.getState());
					target.setRegion(regionModel);
				}
				catch (final UnknownIdentifierException e)
				{
					LOG.warn("Region model with isocode, " + source.getCountry() + "-" + source.getState()
							+ ", is undefined.  It may not be supported by CIS.", e);
				}
			}
		}
	}
