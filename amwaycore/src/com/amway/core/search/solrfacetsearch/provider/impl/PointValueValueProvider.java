...
/**
 * This class will provide the value of PointValue to Solr
 */
public class PointValueValueProvider extends AbstractPropertyFieldValueProvider implements FieldValueProvider, Serializable
{
	private static final Logger LOG = Logger.getLogger(PointValueValueProvider.class);
			...
			final IndexedProperty indexedProperty) throws FieldValueProviderException
	{
		...
		for (final PriceInformation priceInformation : prices)
		{
			if (priceInformation != null)
			{
				...
				{
					Double value = (Double) priceInformation.getQualifierValue("pointValue");

					List rangeNames = getRangeNameList(indexedProperty, value, currency.getIsocode());
