package com.amway.integration.cis.v3.populators;
...
import com.amway.integration.cis.dms.populators.AbstractDmsPopulator;
import com.amway.integration.cis.v3.constants.AddressSatusEnum;
import com.amway.integration.cis.v3.swagger.Account;
import com.amway.integration.cis.v3.swagger.AccountBalance;
import com.amway.integration.cis.v3.swagger.AccountMaster;
import com.amway.integration.cis.v3.swagger.Address;
import com.amway.integration.cis.v3.swagger.AddressContactUsage;
import com.amway.integration.cis.v3.swagger.BlockPrivilege;
import com.amway.integration.cis.v3.swagger.ContactUsage;
import com.amway.integration.cis.v3.swagger.Ecomm;
import com.amway.integration.cis.v3.swagger.MissingInfoDetail;
import com.amway.integration.cis.v3.swagger.NameInfo;
import com.amway.integration.cis.v3.swagger.Party;
import com.amway.integration.cis.v3.swagger.PartyMaster;
import com.amway.integration.cis.v3.swagger.PartyName;
import com.amway.integration.cis.v3.swagger.PersonalId;
import com.amway.integration.cis.v3.swagger.Phone;
import com.amway.integration.cis.v3.swagger.Sponsor;
import com.amway.integration.cis.v3.swagger.Tax;


/**
 * Populator populates data from GET Account MDMSv3 call into an old data object {@link AmwayProfileResponseData}
 * (compatible with MDMSv2).
 * This is a adapted replica of {@link com.amway.integration.cis.dms.populators.DmsAmwayProfileServiceOutputPopulator}.
 */
public class AmwayProfileOutputPopulator extends AbstractDmsPopulator implements Populator<Account, AmwayProfileResponseData>
{
	private static final Logger LOG = Logger.getLogger(AmwayProfileOutputPopulator.class);
	...
	private List<PartyDetailsData> populatePartyDetails(Account source)
	{
		...
		for (final Party party : source.getPartyList())
		{
			...
			partyDetailsData.setMissingInfoList(convertMissingInfoList(source.getMissingInfoList()));
	...
	private PartyPhoneDetailsRequestData populatePhoneList(List<Phone> phoneList)
	{
		...
		for (final Phone partyPhoneData : phoneList)
		{
			...
			phoneMasterData.setSmsCapableFlag(formatBoolean(partyPhoneData.getSmsCapableFlag()));
			...
			phoneMasterData.setDayFlag(formatBoolean(partyPhoneData.getDayFlag()));
			phoneMasterData.setEveningFlag(formatBoolean(partyPhoneData.getEveningFlag()));
			...
			for (final ContactUsage usageData : partyPhoneData.getUsageList())
			{
				...
				usageRequestData.setPrimaryFlag(formatBoolean(usageData.getPrimaryFlag()));

				if (Boolean.TRUE.equals(usageData.getPrimaryFlag()))
				{
					isPrimary = true;
				}
	...
	private PartyEcommDetailsResponseData populateEcommDetails(List<Ecomm> ecommList)
	{
		...
		for (final Ecomm partyEcommData : ecommList)
		{
			...
			for (final ContactUsage usageData : partyEcommData.getUsageList())
			{
				...
				final UsageRequestData usageRequestData = new UsageRequestData();
				usageRequestData.setContactPointPurposeCd(usageData.getContactPointPurposeCd());
				boolean primaryLocal = Boolean.TRUE.equals(usageData.getPrimaryFlag());
	...
	public List<AddressData> populateAddressList(final List<Address> addressList)
	{
		...
		return Optional.ofNullable(addressList)
				...
				.stream()
				.filter(address -> AddressSatusEnum.VALID.code.equals(address.getStatusCd()))
	...
	protected void setAddressType(final AddressData addressData, final AddressContactUsage usageData){
		...
		//final boolean isPrimary = LynxCoreConstants.MagicDmsBoolean.TRUE.equalsIgnoreCase(usageData.getPrimaryFlag());
		final boolean isPrimary = usageData.getPrimaryFlag();
